...
void dfs(int element, vector<bool>& visited, vector<vector<int>>& a){
    visited[element] = true;
    for(int i=0; i<a.size(); i++){
        if(!visited[i] && a[element][i] ==1) {
            dfs(i, visited, a);
        }
    }
}
...
void solve() {
    ...
    for(auto &c:a) {
        result += c;
        mini = min(abs(c), abs(mini));
    }
    if(k>0 && k%2 != 0) result -= 2* mini;
    cout << result << endl;
