...
unordered_map<string, ll> mpi;
ll fun(string& s, ll x, ll y, ll index){
    if(index >= s.size()-1) return 0;
    
    string key = s.substr(index, y-index+1);
    if(mpi.find(key) != mpi.end()) return mpi[key];
    
    if(s[index] == s[index+1]) return mpi[key] =  1 + fun(s,x,y,index+1);
    else return mpi[key] = fun(s,x,y,index+1);
}
