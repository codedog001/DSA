...
void solve() {
    ...
    sort(a.begin(), a.end());
    ll arraySumIndex = n-(2*k), arraySum = 0;
    for(int i=0; i<=arraySumIndex; i++) arraySum += a[i];
    
    ll remSum = 0;
    for(int i=arraySumIndex+1, j=n-k+1; i<=n-k && j<=n; i++, j++){
        remSum += (int)  (((1.0)* a[i])/a[j]);
    }
    
    cout << remSum + arraySum << endl;
